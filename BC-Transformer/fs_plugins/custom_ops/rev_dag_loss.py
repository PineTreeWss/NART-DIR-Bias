...
class ReverseDagLossFunc(Function):
    config = 1
    config1 = 2
    config2 = 2
...
reverse_dag_loss = ReverseDagLossFunc.apply

class ReverseDagBestAlignmentFunc(Function):
    config = 1
...
reverse_dag_best_alignment = ReverseDagBestAlignmentFunc.apply

class ReverseDagLogsoftmaxGatherFunc(Function):
...
reverse_dag_logsoftmax_gather_inplace = ReverseDagLogsoftmaxGatherFunc.apply
...
def loop_function_noempty_max(last_f: Tensor, links: Tensor, match: Tensor) -> Tensor:
    ...
    return f_next
@jit.script
def select_number_index(match_all: Tensor, output_length: Tensor, target_length: Tensor) -> Tensor:
    bsz = match_all.shape[0]
    t_return = torch.zeros(bsz, dtype=match_all.dtype, device=match_all.device)
    for i in range(bsz):
        t_return[i] = match_all[i][output_length[i]][target_length[i]]
    return t_return
def finit_assign(f_init: Tensor, output_length: Tensor, assign_num: Tensor ) -> Tensor:
    bsz = f_init.shape[0]
    for i in range(bsz):
        f_init[i][output_length[i]-1] = assign_num[i]
    return f_init
def reverse_torch_dag_loss(match_all, links, output_length, target_length):
    r"""
    Function to calculate the dag loss.
    Input:
        match_all (torch.FloatTensor or torch.HalfTensor):
    ...
    init_likehood = select_number_index(match_all, output_length-1,target_length-1)
    f_init = finit_assign(f_init,output_length,init_likehood)
    #f_init[:, output_length-1, 0] = select_number_index(match_all, output_length-1,target_length-1)
    ...
    for k in range(tarlen - 2, -1, -1):
        #update_mask = k<output_length
        f_now = loop_function_noempty(f_arr[-1], links, match_all_chunk[k])
        eq_mask = (target_length-1).eq(k).unsqueeze(1).repeat(1,prelen).unsqueeze(-1)
        inf_mask = f_now.eq(float('-inf'))
        f_now.masked_fill_(mask = eq_mask & inf_mask,value=float('-inf'))
        f_now.masked_scatter_(mask = eq_mask,source=f_init)
        '''print('k')
        print(k)
        #print('eq mask')
        #print(eq_mask[-1])
        ...
        #print('links')
        #print(links)
        print('match all chunk')
        print(match_all_chunk[k])
        print('f now')
        print(f_now[-1])'''
        f_arr.append(f_now)
    #exit()
    ...
    for item in f_arr:
        print(item)
    exit()'''
    loss_result = (torch.cat(f_arr, -1)[range(batch_size), 0, -1])
    #print(loss_result)
    #input()
    return loss_result
...
def __torch_max_loss(match_all, links, output_length, target_length):
    ...
    #print(torch.index_select(match_all,1,output_length-1).shape)
    #print(match_all[:, output_length-1,target_length-1].shape)
    init_likehood = select_number_index(match_all, output_length-1,target_length-1)
    #returns the match_all selection according the corresponding output_length and target_lenth
    f_init = finit_assign(f_init,output_length,init_likehood)
    #print(match_all[0,:,-1])
    #f_init[:, output_length-1, 0] = match_all[:, output_length-1,target_length-1]
    #print(target_length-1)
    #print(f_init[0])
    #print(match_all)
    f_arr.append(f_init)
    match_arr = torch.chunk(match_all, tarlen, -1)
    #print('initial f init:')
    #print(f_init[-1])
    #print(match_all[-1,output_length[-1]-1,:])
    for i in range(tarlen - 2, -1, -1):
        #print('i = ',i)
        #print('tgt_length = ',target_length)
        f_now = loop_function_noempty_max(f_arr[-1], links, match_arr[i])
        eq_mask = (target_length-1).eq(i).unsqueeze(1).repeat(1,prelen).unsqueeze(-1)
        #print(eq_mask[-1])
        #print(f_now[-1])
        inf_mask = f_now.eq(float('-inf'))
        #print(f_now[-1])
        f_now.masked_fill_(mask = eq_mask & inf_mask,value=float('-inf'))
        #print(f_now[-1])
        f_now.masked_scatter_(mask = eq_mask,source=torch.masked_select(input=f_init,mask=eq_mask))
        #print(f_now[-1])
        f_arr.append(f_now)

    alllogprob = torch.cat(f_arr, -1)[range(batch_size), 0 , -1]
    return alllogprob

def reverse_torch_dag_best_alignment(match_all, links, output_length, target_length):
    r"""
    Function to obtain the alignment between prediction and reference
    Input:
        match_all (torch.FloatTensor or torch.HalfTensor):
   ...
   # print('matchgrad:')
   # for item in matchgrad:
   #     print(item)
    pathvalue, path = matchgrad.max(dim=1)
    #print('pathvalue')
    #for item in pathvalue:
    #    print(item)
    #print('path')
    #for item in path:
    #    print(item)
...
def reverse_torch_dag_logsoftmax_gather_inplace(word_ins_out, select_idx):
