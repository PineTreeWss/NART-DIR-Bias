...
def __torch_max_loss(match_all, links, output_length, target_length):
    #Returns the likehood of the most probable link 
    match_all = match_all.transpose(1, 2)
    batch_size, prelen, tarlen = match_all.shape
    assert links.shape[1] == links.shape[2], "links should be batch_size * prelen * prelen"
    #print('match all')
    ...
    '''print('cat')
    print(torch.cat(f_arr, -1)[0])
    print('cat_item')
    for item in torch.cat(f_arr,-1)[0]:
        print(item)
    print('cat_shape')
    print(torch.cat(f_arr, -1).shape)
    print('output_length')
    print(output_length)
    print('target_length')
    print(target_length)
    print(torch.cat(f_arr, -1)[range(batch_size), output_length - 1, target_length - 1])
    print('alllogprob')
    print(alllogprob)'''
    return alllogprob
