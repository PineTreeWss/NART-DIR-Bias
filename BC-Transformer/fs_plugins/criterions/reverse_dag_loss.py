...
##########################################################################
#2023/04/19
#This code is a test script, which aims at implement a reverse DAG with the torch implementation.
#Should be coped with reverse DAG Search
##########################################################################
import math
import re
import logging
...
from ..custom_ops import dag_loss, dag_best_alignment, dag_logsoftmax_gather_inplace, reverse_torch_dag_loss, reverse_torch_dag_best_alignment, reverse_torch_dag_logsoftmax_gather_inplace
...
@register_criterion("nat_reverse_dag_loss")
class NATDAGLoss(FairseqCriterion):
    ...
    def _compute_dag_loss(self, outputs, output_masks, targets, target_masks, links, label_smoothing=0.0, name="loss",
        ...
        if self.cfg.torch_dag_logsoftmax_gather:
            outputs, match_all = reverse_torch_dag_logsoftmax_gather_inplace(outputs, targets.unsqueeze(1).expand(-1, prelen, -1))
        else:
            print('Cuda version of reversed-DAG not implemented,program terminated.')
            exit()
    ...
    def forward(self, model, sample, reduce=True):
        ...
        def glat_function(model, word_ins_out, tgt_tokens, prev_output_tokens, glat, links=None):
            ...
            if self.cfg.torch_dag_logsoftmax_gather:
                word_ins_out, match = reverse_torch_dag_logsoftmax_gather_inplace(word_ins_out, tgt_tokens.unsqueeze(1).expand(-1, prelen, -1))
            else:
