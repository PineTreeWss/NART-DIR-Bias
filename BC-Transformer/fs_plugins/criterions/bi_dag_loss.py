...
from ..custom_ops import dag_loss, rev_dag_loss,dag_best_alignment, dag_logsoftmax_gather_inplace, torch_dag_loss, torch_dag_best_alignment, torch_dag_logsoftmax_gather_inplace,  reverse_torch_dag_loss, reverse_torch_dag_best_alignment, reverse_torch_dag_logsoftmax_gather_inplace
...
@register_criterion("bi_nat_dag_loss")
class BiNATDAGLoss(FairseqCriterion):
    ...
    def _compute_dag_loss(self, outputs, output_masks, targets, target_masks, links, label_smoothing=0.0, name="loss",
                factor=1.0, dir = 'n', matchmask=None, keep_word_mask=None, model=None):
        ...
        if self.cfg.torch_dag_loss:
            ...
            if dir == 'r':
                ...
                loss_result = reverse_torch_dag_loss(match_all, links, output_length, target_length)
        else:
    ...
    def forward(self, model, sample, reduce=True):
        ...
        def glat_function(model, word_ins_out, tgt_tokens, prev_output_tokens, glat, dir='n',links=None):
        ...
        n_outputs,r_outputs = model(src_tokens, src_lengths, prev_output_tokens, tgt_tokens, glat, glat_function)

        losses = []
        # DAG loss
        n_losses = self._compute_dag_loss(
            n_outputs["word_ins"].get("out"),
            prev_output_tokens.ne(self.task.tgt_dict.pad()),
            n_outputs["word_ins"].get("tgt"),
            n_outputs["word_ins"].get("mask", None),
            n_outputs["links"],
            name="dag-loss",
            factor=1,
            dir = 'n' ,
            matchmask= n_outputs.get('matchmask', None),
            keep_word_mask=n_outputs.get('keep_word_mask', None),
            model=model
        )
        ...
        _losses = {"name": "loss", "loss": n_losses["loss"] * 0.5 + r_losses["loss"] * 0.5, "nll_loss":  n_losses["nll_loss"].detach() * 0.5  + r_losses["nll_loss"].detach() * 0.5,
                "factor": n_losses["factor"], "ntokens": n_losses["ntokens"]+ r_losses["ntokens"],  "nvalidtokens": n_losses["nvalidtokens"]+r_losses["nvalidtokens"], "nsentences": n_losses["nsentences"]+r_losses["nsentences"],
                "loss_nofactor": n_losses["loss_nofactor"], "invalid_nsentences":n_losses["invalid_nsentences"]+r_losses["invalid_nsentences"]}
        losses += [_losses]
        n_losses["loss"] = n_losses["loss"].detach()
        r_losses["loss"] = r_losses["loss"].detach()
        dag_nll_loss = _losses["nll_loss"].detach()
        nsentences = n_losses["nsentences"]
        ntokens = n_losses["ntokens"]
        nvalidtokens = n_losses["nvalidtokens"]
        invalid_nsentences = n_losses["invalid_nsentences"]
        #length
        _losses = self._compute_loss(
            n_outputs["length"].get("out"),
            n_outputs["length"].get("tgt"),
            None,
            0,
            name="length-loss",
            factor=n_outputs["length"]["factor"], )
        losses += [_losses]
        ...
        logging_output = {
            "loss": loss.data,
            "n_loss": n_losses["loss"],
            "r_loss": r_losses["loss"],
            ...
            "glat_acc": n_outputs.get("glat_accu", 0),
            "glat_keep": n_outputs.get("glat_keep", 0),
        }
